        ...
        {% elif pagina == 'crud' %}
            ...
            async function carregarDados() {
                try {
                    ...
                    // If this is the products page, fetch categories to map IDs to names
                    let categorias = {};
                    if ('{{ endpoint }}' === 'produtos') {
                        const catResponse = await fetch('/api/categorias');
                        const catData = await catResponse.json();
                        categorias = Object.fromEntries(catData.map(cat => [cat.id, cat.nome]));
                    }
                        ...
                        <tr>
                            ...
                            <td>${
                                {% if endpoint == 'produtos' and campo.id == 'categoria_id' %}
                                    categorias[item.{{ campo.id }}] || item.{{ campo.id }}
                                {% else %}
                                    item.{{ campo.id }}
                                {% endif %}
                            }</td>
